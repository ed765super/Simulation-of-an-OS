...
//         programSize = 0
Simulator::Simulator(int requestedMemory, int pStackSize,
		     int pSwapSize, int pFitType){
  PC = 0;
  ...
  swapSpace = new unsigned int[pSwapSize];
  fitType = pFitType;
...
Simulator::Simulator(const Simulator & S) {
  ...
  swapSpace = new unsigned int[numWords];
  // get deep copy of swapSpace
  for (int index = 0; index < numWords; index ++){
    memory[index] = S.getWord(index); 
  }
...
Simulator & Simulator::operator= (const Simulator & S) {
  ...
  stackSize = new unsigned int[numStackSize];
  // get deep copy of stackSize
  for (int index = 0; index < numStackSize; index ++){
    stackSize[index] = S.getStackWord(index); 
  }
  stackSize = new unsigned int[swapSize];
  // get deep copy of swapSpace
  for (int index = 0; index < swapSize; index ++){
    stackSize[index] = S.getStackWord(index); 
  }
  sim_registers = S.getRegisters(); // get deep copy of registers
  halt = S.haltStatus();
...
//PRE:
//POST: returns the stackSpace
unsigned int* Simulator::getSwapSpace() const{
  return swapSpace;
}
...
// PRE:
// POST: returns swapSize
int Simulator::getSwapSize() const {
  return swapSize;
}
...
//PRE: 
//POST: handles/manipulates of memory, swap, the frame
//      tables, and/or the page tables s.t. they represent any
//      modifications made by the OS.
void Simulator::addPageToMemOrSwapOrCout(OS & myOS){
  PCB* currPCBptr = myOS.getCurrPCB();
  MyString currProgName = currPCBptr->getProgName();
  char* currProgString = currProgName.getstring();
  bool inMemory = false;//true iff the page is already in memory
  bool memFull = false;//true iff memory is full
  int pageSize = myOS.getPageSize();

  frameEntry* memFT = myOS.getMemFrameTable();
  int numMemFrameTables = myOS.getMemFTsize();
  frameEntry* swapFT = myOS.getSwapFrameTable();
  int numSwapFrameTables = myOS.getSwapFTsize();
  int pagesOfMemory = numWords / pageSize;
  int pagesOfSwap = swapSize / pageSize;

  if (!(pagesOfMemory == numMemFrameTables)){
    //ASSERT: Memory is not full
    handleLoadHelper(currProgString, myOS);
    //ASSERT: the next page has been added to memory
    int currID = currPCBptr->getProcessID();
    int oldSizeofMemTable = myOS.getMemFTsize();
    int newSizeOfMemFT = oldSizeofMemTable + 1;
    myOS.setMemFTsize(newSizeOfMemFT);
    /*update frame table*/
    /*update page table*/
  }
  else{
...
bool Simulator::executeNext(OS & myOS){
  unsigned int currentWord = memory[PC/BYTE];
  int opcode = getNibble(currentWord, 0);
  int pageSize = myOS.getPageSize();
  PCB* currPCBptr = myOS.getCurrPCB();
  if (PC % pageSize == 0){
    //ASSERT: new page needs to be requested
    addPageToMemOrSwapOrCout(myOS);
    //ASSERT: The handling and/or manipulation of memory, swap, the frame
    //        tables, and/or the page tables for this process have been
    //        completed
  }
  incrementPC();
  bool error = false; 
  if (opcode == 0){
    ...
    /*something happens here regarding page faulting*/
  }
  else if (opcode == 4){
    //ASSERT: opcode is SW
    error = executeSw(currentWord);
    PCB* currPCBptr = myOS.getCurrPCB();
    LinkedList<pageEntry> currPT = currPCBptr->getPageTable();
    pageEntry* currentPageEntry = currPT.getTail();
    currentPageEntry->setDirty(true);
    //ASSERT: current page is dirty
    currPCBptr->setPageTable(currPT);
  }
  else if (opcode == 5){
...
bool Simulator::handleLoad(char* fileName, OS & myOS){
  ...
  ifstream* file;
  file->open(trueFileNameString, ios::in);
  if (file != NULL) {
    //ASSERT: valid file name
    int numLines = countBinLines(*file);

    //~~~~~~~~~~~~~~~~~~1~~~~~~~~~~~~~~~~~~~~~~~~~
    int VPN = myOS.getMemFTsize();
    //ASSERT: found an integer representing the page before the
    //        first free page in memory

    //~~~~~~~~~~~~~~~~~~~~~~~~~~~2~~~~~~~~~~~~~~~~
    findSpace(numLines, myOS);
    //ASSERT: space for the incoming program has been found or the OS
    //        contains an ERROR
    //~~~~~~~~~~~~~~~~~3~~~~~~~~~~~~~~~~~~~~~~
    handleLoadHelper(trueFileNameString, myOS);
...
bool Simulator::handleStep(int requestedSteps, OS & myOS){
  ...
  curr_PCBptr->setProcessStatus(1);
  LinkedList<Register> currPCBRegisters = curr_PCBptr->getRegisters();
  ...
  while ((steps < requestedSteps) && (!halt) && (!error)){
    ...
    curr_PCBptr->setNumTimeSlices(curr_PCBptr->getNumTimeSlices() + 1);
    //ASSERT: the number of timeslices used by the current process has been
    //incramented by 1
    curr_PCBptr->setProcessStatus(0);
    if (halt){
...
//         each set of 8 bytes placed into an unsigned int,
//         all unsigned ints placed into memory.
void Simulator::handleLoadHelper(char* fileName, OS & myOS) {
  ifstream file(fileName); // this file holds the program
  int pageSize = myOS.getPageSize();
  unsigned int currentWord = 0; // this accumulates a word at a time
  unsigned int mask = 0xff;
  int wordsAdded = 0; //will hold the number of words added to the page
  ...
  while ((!file.eof()) && (wordsAdded != pageSize)) {
...
//PRE: numLines nust be an integer that represents the number of lines in the
//     program. myOS represents the OS object
//POST: Attempts to find space for the program and stack. if it could,
//      Allocates that space into freeBlocks. if it cant OS contains an error
//      msg.
void findSpace(int numLines, OS & myOS){
  int pageSize = myOS.getPageSize();
  Block progBlock;
  progBlock.setStart(0);
  progBlock.setEnd(pageSize);
  //The block representing the amount of RAM this page of the program block
  //is taking up has been created
  Block stackBlock;
  stackBlock.setStart(0);
  stackBlock.setEnd(pageSize);
  //The block representing the amount of RAM this page of the stack block
  //is taking up has been created
  ...
  if (foundSpace){
    ...
    pageTable(bool /*progOrStack*/, int /*VPN*/, bool /*dirty*/,
	      bool /*memOrSwap*/);
    PCB currPCB(0, fileName, findProgBlock, findStackBlock,
		curr_ID, sim_registers, pageTable, currPageFaults,
		numTimeSlices, file);
