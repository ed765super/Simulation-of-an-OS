...
void evaluate_command(LinkedList<MyString> tokenList, Simulator & sim,
  ...
  else if(isEqualString(command->getstring(), PAGES)){
    myOS.handlePages();
    if (tokenList.getSize() == 2){
      //user gave a start arguement
      MyString* MYPID = tokenList.getNth(1);
      char* PIDString = MYPID->getstring();
      int PID = strToInt(PIDString);
      myOS.handlePages(PID);
    }
  }
