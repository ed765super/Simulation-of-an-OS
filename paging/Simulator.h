...
class Simulator {
 private:
  ...
  unsigned int* stackSize; //this array functions as the size that the
			   ...
			   //for the stack
  unsigned int* swapSpace; // this array functions as the swap space for
                        // the simulator
  int numStackSize; // number of unsigned ints in stackSize
  int swapSize; //number of unsigned ints in swap
 ...
 public:
  ...
  //         programSize = 0.
  Simulator(int requestedMemory, int pStackSize,
  ...
  unsigned int* getSwapSpace() const;
  ...
  void addPageToMemOrSwapOrCout(OS & myOS);
  ...
  ///     n % 4 = 0
  //      pageSize must be the size of any page
  // POST: decodes chars into bytes,
  //         each set of 8 bytes placed into an unsigned int,
  //         a page unsigned ints placed into memory
  void handleLoadHelper(char* fileName, OS & myOS);
  // PRE:
