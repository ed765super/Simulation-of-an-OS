...
bool checkToken(LinkedList<MyString> tokenList, int sizeofmemory){
  ...
  else if(isEqualString(commandString, PAGES)){
    validToken = true;
    if (tokenList.getSize() == 2){
      //There is an additional parameter with pages
      MyString* PID = tokenList.getNth(1);
      char* PIDstring = PID->getstring();
      //ASSERT: PID is the representation address where the user
      //wants to start printing
      int PIDSize = PID->length();//Holds how many digits are in
      //startSize
      int curr_digit = 0;
      bool PIDIsDigit;//Will turn true iff the first operand of MEM is a
      //digit
      while (curr_digit < PIDSize){
	PIDIsDigit = isDigit(PIDstring[curr_digit]);
	curr_digit++;
      }
      if (PIDIsDigit){
	if ((*PIDstring >= CHAR0) &&
	    (*PIDstring < sizeofmemory)){
	  //ASSERT: the start is valid
	  validToken = true;
	}
	else{
