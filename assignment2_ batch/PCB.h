...
class PCB{
 ...
 private:
  ...
  Register stackPointer; //initializes to the start of this process
 ...
 public:
  ...
  //ACCESSOR
  //PRE:
  //POST: returns the actual PC
  int getActualPC() const;
  ...
  //would be a valid PC.
  //POST: sets the actual PC to pPC
  void setActualPC(int pPC);
  ...
  //PRE: this object must be defined. pStackPointer must be an integer
  //that represents the stack pointer of this PCB's stack space
  //POST: modifies the stack pointer
  void setStackPointer(Register pStackPointer);
