...
void OS::handleFreeMem(){
  cout << "FreeBlocks: " << freeBlocks << endl;
}
...
//the queue.
void OS::handleJobs(){
  if(PCB_list.getSize() == 0){
    //ASSERT: There is noting on the running queue
    cout <<"There is nothing on the running queue at this moment" << endl;
  }
  else{
    cout << PCB_list << endl;
  }
