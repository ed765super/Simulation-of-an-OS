...
bool Simulator::handleLoad(char* fileName, OS & myOS){
  ...
  if (file != NULL) {
	...
	sim_registers.update(index, currRegister); 
      ...
      int indexOfSP = 13; //the index of stack pointer
      Register* currSP = getRegister(indexOfSP);
...
bool Simulator::handleStep(int requestedSteps, OS & myOS){
  ...
  relativePC = relativePC + (steps * BYTE);
  ...
  if (halt){
    ...
    if (runningQueue.getSize() > 0){
      ...
      Register PCB_StackPointer = nextProgram->getStackPointer();
      int indexOfSP = 13; //the index of stack pointer
      Register* currSP = getRegister(indexOfSP);
      currSP->setValue(PCB_StackPointer.getValue());
      //currSP is now the value of the next program to be executed.
...
//       if no runtime error occured return true
//       else return false 
bool Simulator::handleRun(OS & myOS){
  bool error = false;
  while (!halt && !error) {
    error = executeNext();
  }
